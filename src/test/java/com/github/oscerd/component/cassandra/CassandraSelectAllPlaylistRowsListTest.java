...
public class CassandraSelectAllPlaylistRowsListTest extends CassandraPlaylistBaseTest {

    @Test
    public void testSelectAllPlaylistRowsList() throws IOException, InterruptedException {
        ...
        List<Integer> songOrderList = new ArrayList<Integer>();
        Iterator it = result.iterator();
        while (it.hasNext()){
        	Row row = (Row) it.next();
        	songOrderList.add(row.getInt("song_order"));
        }
        List<Integer> tmp = new ArrayList<Integer>();
        tmp.add(3);
        tmp.add(2);
        tmp.add(1);
        assertEquals(tmp, songOrderList);
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            public void configure() {
                from("direct:in")
                    .to("cassandra:cassandraConnection?keyspace=simplex&table=playlists&operation=selectAllWhere&format=rowsList")
                    .to("mock:result");
            }
        };
    }
